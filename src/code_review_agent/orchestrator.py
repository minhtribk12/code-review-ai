...
def _get_max_severity(findings: list[Finding], *, default: Severity = Severity.LOW) -> Severity:
...
class Orchestrator:
    ...
    def cancel(self) -> None:
        """Signal the orchestrator to stop pending agents.

        Completed agents' results are preserved. Synthesis and validation
        still run on partial results so the caller gets a usable report.
        """
        self._cancelled.set()

    def abort(self) -> None:
        """Signal the orchestrator to stop all processing immediately.

        Unlike ``cancel()``, abort also skips synthesis and validation.
        The caller receives a minimal report with whatever agents completed.
        """
        self._aborted.set()
        self._cancelled.set()
    ...
    def _synthesize(self, *, agent_results: list[AgentResult]) -> SynthesisResponse:
        ...
        max_severity = _get_max_severity(all_findings)
