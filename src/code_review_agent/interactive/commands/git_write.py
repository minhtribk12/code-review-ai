...
def cmd_cd(args: list[str], session: SessionState) -> None:
    ...
    if not args:
        ...
        target = Path(os.environ.get("OLDPWD", "."))
    else:
    ...
    old_dir = Path.cwd()
    os.chdir(target)
    os.environ["OLDPWD"] = str(old_dir)

    # Clear active repo when switching directories (local repo changed)
    if session.active_repo and session.active_repo_source == "local":
