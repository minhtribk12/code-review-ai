...
COMMAND_HELP: dict[str, list[tuple[str, str]]] = {
    ...
    "Config": [
        ...
        ("config keys", "API key manager (view, edit, sync, delete)"),
    ...
    "News": [
        ("news <topic>", "Multi-source brief: HN + Reddit + Web (e.g., news ai)"),
        ("news 30days <topic>", "Deep 30-day research across all sources"),
        ("news list", "List available preset domains (40+ built-in)"),
        ("news stats", "Show stats + cache size per domain"),
        ("news add <name> <url>", "Add a custom RSS feed (persisted)"),
        ("news remove <name>", "Remove a custom RSS feed"),
        ("news refresh", "Clear cache and force re-fetch"),
        ("news cleanup", "Delete unsaved articles older than 30 days"),
        ("read-news [domain]", "Browse articles (S sort, / search, F filter)"),
    ],
    "Meta": [
        ...
        ("Ctrl+A", "Quick agent selector (persisted to config.yaml)"),
        ("Ctrl+P", "Quick provider selector (persisted to config.yaml)"),
