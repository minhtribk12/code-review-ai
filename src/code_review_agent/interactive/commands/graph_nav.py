...
class GraphState:
    ...
    def refresh_detail(self) -> None:
        """Refresh the detail panel for the current commit."""
...
def _render_header(state: GraphState) -> _Lines:
    ...
    if state.mode == GraphMode.DETAIL:
        lines.append((theme.muted, " [Up/Down] navigate  [Space] close  [q] quit\n"))
...
def run_graph_app(*, count: int | None = None, branches: list[str] | None = None) -> None:
