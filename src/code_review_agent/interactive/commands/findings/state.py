...
class FindingsViewer:
    ...
    def __init__(
        ...
        *,
        ...
        # Sort columns in display order (matches _DEFAULT_VISIBLE left to right).
        # file_line column sorts by file_path attribute.
    ...
    def cycle_sort(self) -> None:
        """Cycle sort forward: first press reverses, second moves to next column."""
        if not self.is_sort_reversed:
            self.is_sort_reversed = True
        else:
            self.is_sort_reversed = False
            self.sort_index = (self.sort_index + 1) % len(self.sort_columns)
        self._apply_sort()

    def cycle_sort_backward(self) -> None:
        """Cycle sort backward: first press reverses, second moves to previous column."""
        if not self.is_sort_reversed:
            self.is_sort_reversed = True
        else:
            self.is_sort_reversed = False
