...
class ReviewStorage:
    ...
    def _migrate(self, conn: sqlite3.Connection) -> None:
        ...
        # Ensure findings table has comment_id column
        if "findings" in tables:
            finding_cols = {
                row[1] for row in conn.execute("PRAGMA table_info(findings)").fetchall()
            }
            if "comment_id" not in finding_cols:
                with contextlib.suppress(sqlite3.OperationalError):
                    conn.execute("ALTER TABLE findings ADD COLUMN comment_id INTEGER")
    ...
    def _backfill_findings(self, conn: sqlite3.Connection) -> None:
        ...
        for review in reviews:
            try:
                report = ReviewReport.model_validate_json(review["report_json"])
                _, pr_number = _parse_github_pr_url(review["pr_url"])
                repo = review["repo"]
                review_id = review["id"]
    ...
    def save(
        ...
        *,
        ...
        url_repo, pr_number = _parse_github_pr_url(report.pr_url)
    ...
    def update_finding_posted(
        ...
        *,
        ...
        """Update posted status and comment ID on a finding."""
        with self._get_connection() as conn:
            if is_posted and comment_id is not None:
                conn.execute(
                    "UPDATE findings SET is_posted = 1, comment_id = ? WHERE id = ?",
                    (comment_id, finding_id),
                )
            elif not is_posted:
                conn.execute(
                    "UPDATE findings SET is_posted = 0, comment_id = NULL WHERE id = ?",
                    (finding_id,),
                )
            else:
                conn.execute(
...
def _parse_github_pr_url(pr_url: str | None) -> tuple[str | None, int | None]:
    """Extract (repo, pr_number) from a GitHub PR URL.

    Example: https://github.com/acme/app/pull/42 -> ('acme/app', 42)
    Returns (None, None) when the URL is missing or unparseable.
    """
    if pr_url is None:
        return None, None
