...
- Two tables: `reviews` (indexed metadata + full `report_json`) and `agent_results` (per-agent stats)
- Indexes on `repo`, `reviewed_at`, `risk_level`, `llm_model`, `agent_name`
- Schema versioning with forward migration (v1 to v2)
- Query methods: `list_reviews`, `get_review`, `get_trends` (aggregated stats), `get_agent_stats` (per-agent performance)
