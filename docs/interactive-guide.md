...
| `s` | Cycle sort column (severity, agent, file, title) |
...
All triage state is in-memory per session (not persisted). Second press
toggles the action off.
...
cra> config save                     # persist session overrides to .env
```
...
Use `config save` to persist to `.env`.
