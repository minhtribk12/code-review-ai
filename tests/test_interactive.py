...
class TestCommandDispatch:
    ...
    def test_exit_warns_unsaved_config(self, session: SessionState) -> None:
        """Exit with unsaved config shows save prompt via _confirm_exit."""
        from code_review_agent.interactive.repl import _confirm_exit

        session.config_overrides["llm_model"] = "new/model"
        with (
            patch("code_review_agent.interactive.repl.console") as mock_console,
            patch("builtins.input", return_value="3"),  # Cancel
        ):
            result = _confirm_exit(session)
        assert result is False
        printed = " ".join(str(c) for c in mock_console.print.call_args_list)
        assert "unsaved" in printed.lower()
...
class TestMetaCommands:
    ...
    def test_version(self, session: SessionState) -> None:
        with patch("code_review_agent.interactive.commands.meta.console") as mock_con:
            cmd_version([], session)
        assert "0.1.3" in str(mock_con.print.call_args)
...
class TestPrReview:
    """Test that pr review fetches diff and starts background review."""

    def test_pr_review_no_diff_shows_message(self, session_with_token: SessionState) -> None:
        with (
            ...
            patch("code_review_agent.interactive.commands.pr_read.console") as mock_con,
        ):
            mock_info.return_value = ("acme", "app", "ghp_token")
        ...
        assert "no diff" in str(mock_con.print.call_args_list).lower()

    def test_pr_review_starts_background(self, session_with_token: SessionState) -> None:
        from code_review_agent.models import DiffFile, DiffStatus, ReviewInput

        diff_file = DiffFile(filename="a.py", patch="+x", status=DiffStatus.MODIFIED)
        ...
        with (
            ...
            patch(
                "code_review_agent.interactive.commands.review_cmd._run_review_on_input",
            ) as mock_run,
        ):
            mock_info.return_value = ("acme", "app", "ghp_token")
            mock_fetch.return_value = review_input
...
class TestPrReviewErrorHandling:
    """Test pr review error cases."""

    def test_pr_review_missing_number(self, session_with_token: SessionState) -> None:
...
class TestReviewStorage:
    ...
    def test_pr_url_parsing(self) -> None:
        from code_review_agent.storage import _parse_github_pr_url

        repo, pr_number = _parse_github_pr_url(
            "https://github.com/acme/app/pull/42",
        )
        assert repo == "acme/app"
        assert pr_number == 42
        assert _parse_github_pr_url(None) == (None, None)
        assert _parse_github_pr_url("https://github.com/acme/app") == (None, None)
