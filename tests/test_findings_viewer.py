...
class TestSort:
    def test_cycle_sort_reverses_first(self, viewer: FindingsViewer) -> None:
        assert viewer.sort_index == 0
        assert viewer.is_sort_reversed is False
        viewer.cycle_sort()
        # First press reverses current column, doesn't move
        assert viewer.sort_index == 0
        assert viewer.is_sort_reversed is True

    def test_cycle_sort_advances_after_reverse(self, viewer: FindingsViewer) -> None:
        viewer.cycle_sort()  # reverse
        assert viewer.is_sort_reversed is True
        viewer.cycle_sort()  # un-reverse + move to next
        assert viewer.sort_index == 1
        assert viewer.is_sort_reversed is False

    def test_sort_wraps_around(self, viewer: FindingsViewer) -> None:
